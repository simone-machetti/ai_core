...
package pe_top_pkg;
    ...
    function automatic int get_pp_gen_pp_size(input int arch, input int mult_type, input int in_width_a, input int in_width_b, input int in_size);
        return pp_gen_pkg::get_pp_size_arch(arch, mult_type, in_width_a, in_width_b, in_size);
    endfunction

    function automatic int get_pp_gen_pp_width(input int arch, input int mult_type, input int in_width_b);
        return pp_gen_pkg::get_pp_width_arch(arch, mult_type, in_width_b);
    endfunction

    function automatic int get_cpr_tree_pp_group(input int arch, input int mult_type, input int in_width_a, input int in_width_b);
        return (arch == pp_gen_pkg::WINOGRAD) ? pp_gen_pkg::get_pp_group_winograd(mult_type, in_width_b)
                                              : pp_gen_pkg::get_pp_group(mult_type, in_width_a);
    endfunction
    ...
    function automatic int get_cpr_tree_out_width(input int arch, input int mult_type, input int in_width_a, input int in_width_b, input int in_size);
        return get_pp_gen_pp_width(arch, mult_type, in_width_b)
             + $clog2(get_pp_gen_pp_size(arch, mult_type, in_width_a, in_width_b, in_size) / get_cpr_tree_pp_group(arch, mult_type, in_width_a, in_width_b))
             + 1
             + (get_cpr_tree_pp_shift(arch, mult_type) * (get_cpr_tree_pp_group(arch, mult_type, in_width_a, in_width_b) - 1));
    endfunction
